...
case "$shard" in
  free_properties|form_definitions|files_configuration|account_memberships|product_definitions) ;;
  *) echo "cleanup shard must be free_properties, form_definitions, files_configuration, account_memberships, or product_definitions" >&2; exit 1 ;;
esac
...
case "$mode" in
  ...
  archive)
    case "$shard:$confirm" in
      free_properties:archive-prefixed-crm-configuration|form_definitions:archive-prefixed-form-definitions|files_configuration:delete-prefixed-files-configuration|account_memberships:delete-prefixed-account-memberships|product_definitions:archive-prefixed-product-definitions) ;;
