...
case "$shard" in
  ...
  *) echo "cleanup shard must be free_properties or form_definitions" >&2; exit 1 ;;
esac
...
case "$mode" in
  ...
  archive)
    case "$shard:$confirm" in
      free_properties:archive-prefixed-crm-configuration|form_definitions:archive-prefixed-form-definitions) ;;
